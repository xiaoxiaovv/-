var highlightNewsListCtrl = function ($rootScope, $scope, $http, $state, $sce, $stateParams) {
    $scope.modalName = $stateParams.modal;
    $scope.needShow = false;
    $scope.highlightShow = true;
    ...
    $scope.hightlightToggleAllNewsList = function () {
        ...
        $scope.hightLightNews = _.filter($scope.dataList, {'active' : true});
    };
    /*单选*/
    $scope.hightlightToggleNewsList = function () {
        ...
        $scope.active = false;
        $scope.hightLightNews = _.filter($scope.dataList, {'active' : true});
        if($scope.hightLightNews.length == $scope.dataList.length){
            $scope.active = true;
        }
    };
    /*跳详情*/
    $scope.gotoDetail = function ($event, newsId) {
        $rootScope.writeNewsId($rootScope.user.userName, newsId, 'highlightNewsMore');
        $(".deleteNews").remove();
    ...
    $scope.pushAll = function(){
        var ids = [];
        var newsIds = { "newsIds" : [] };
        _.forEach($scope.hightLightNews, function (v, i) {
            ids.push($scope.hightLightNews[i].id);
        });
        if (ids.length == 0){
            ...
            bjj.http.ng.postBody($scope, $http, '/api/capture/news/push?_method=post', JSON.stringify(
                ...
                _.forEach($scope.hightLightNews, function (v, i) {
                    for(var key in newsIds) {
                        if(($scope.hightLightNews[i].id) == newsIds[key]) {
                            $scope.hightLightNews[i].isPush = true;
                        }
                    }
                    $scope.hightLightNews[i].isPush = true;
                });
    ...
    $scope.detailFavoriteGroup = function($event, newsId, groupId) {
        $event.stopPropagation();
        detailAddFavorite($scope, $http, newsId, groupId, function(res) {
            $(".detailFavorite").hide();
            $scope.hightLightNews.isCollection = true;
            for(var n in $scope.dataList){
                if(newsId == $scope.dataList[n].id){
                    $scope.dataList[n].isCollection = true;
                }
...
var getNewsListData = function($scope, $http){
    bjj.http.ng.get($scope, $http,'/api/capture/dashboard/highlight/moreNews', {
        ...
        $scope.readNewsId($scope.user.userName, 'highlightNewsMore', list);
...
var addFavorite = function($scope, $http, newsIds, groupId, callback) {
    bjj.http.ng.postBody($scope, $http, '/api/capture/news/favorite?_method=post', JSON.stringify({
        newsIds : [newsIds],
        groupId : groupId
    }), function (res) {
...
var allAddFavorite = function($scope, $http, groupId, callback) {
    ...
    bjj.http.ng.postBody($scope, $http, '/api/capture/news/favorite?_method=post', JSON.stringify({
        ...
        _.forEach($scope.hightLightNews, function (v, i) {
            for(var key in newsIds) {
                if(($scope.hightLightNews[i].id) == newsIds[key]) {
                    $scope.hightLightNews[i].isCollection = true;
                }
            }
            $scope.hightLightNews[i].isCollection = true;
        });
