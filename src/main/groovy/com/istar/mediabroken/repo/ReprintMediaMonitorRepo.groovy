...
@Repository
@Slf4j
class ReprintMediaMonitorRepo {
    @Autowired
    MongoHolder mongo
    @Autowired
    EsHolder elasticsearch
    @Value('${es.index.reprintMediaMonitor}')
    String index

    List getReprintMediaMonitorFromEs(List<Site> siteList, int hot, Date startTime, Date endTime,
                                      int orientation, boolean hasPic, int order, String keyWords, int pageSize, int offset) {
        String[] includeList = ["id", "author", "poster", "siteName", "contentAbstract", "title", "cover", "imgUrls", "reprintCount", "captureTime", "url", "firstPublishTime", "firstPublishSiteName", "simhash", "publishTime", "createTime"]
        String[] excludeList = []
            ...
            switch (site.siteType) {
                case Site.SITE_TYPE_WEBSITE:
                    siteQueryBuilder.should(QueryBuilders.boolQuery()
                            .must(QueryBuilders.prefixQuery("firstDomainReverseUrl", (null == site.domainReverse || site.domainReverse == "") ? UrlUtils.getReverseDomainFromUrl(site.websiteDomain) : site.domainReverse))
                            .must(QueryBuilders.termQuery("firstPublishNewsType", 1)))
                    break

                case Site.SITE_TYPE_WECHAT:
                    siteQueryBuilder.should(QueryBuilders.boolQuery().must(QueryBuilders.termQuery("firstPublishSiteName", site.websiteName))
                            .must(QueryBuilders.termQuery("firstPublishNewsType", 6)))
                    break
            }
        }
        if (siteList.size() > 0) {
        ...
        SearchRequestBuilder prepareSearch = elasticsearch.client.prepareSearch(index)
                ...
                .setFrom(offset)
                .setSize(pageSize)
                .addSort((order == 1 ? "publishTime" : "firstPublishTime"), SortOrder.DESC)
