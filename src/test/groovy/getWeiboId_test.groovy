...
List getUrl(String basePath) {
    def readLines = FileUtils.readLines(new File(basePath + "weiboURL.txt"), 'utf-8')
    def tmpFile = new File(basePath + "tmp.txt")
    if( tmpFile.exists() ){
        def tmp = FileUtils.readLines(new File(basePath + "Tmp.txt"), 'utf-8')
        def list = readLines - tmp //求差集
        return list
    }else
        return readLines
}
    ...
    while (true) {
        ...
        try {
            result = HttpHelper.doGetArray("https://api.weibo.com/2/statuses/count.json", [
                    access_token: getAccessToken(index),
            ...
            FileUtils.writeLines(new File('C:\\Users\\ZHXG\\Desktop\\tmp.txt'), urlList, true)
...
static def getAccessToken(int index) {
    def tokenList = ["2.00sf1uTG0aC8eg7d9ba794d9SDOYBC",
